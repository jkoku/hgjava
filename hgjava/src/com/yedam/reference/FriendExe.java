...
public class FriendExe {
	...
	public static void main(String[] args) {
		...
		boolean run = true; //사용자가 종료라는 말을 하기 전까지는 반복하려고. 
		Friend [] friends = new Friend[5]; //크기 다섯개짜리 주소는 friends에 담아둠. friends의 몇 번째 
		// {null,null,null,null,null} 객체값의 현재 모양
		
		int score = 0;
		double weight = 0;
		while(run) {
			System.out.println("1.등록 2.조회 3.수정 4.삭제 5.점수조회 6.분석 9.종료");
			//5.점수조회는 입력한 점수를 기준으로 이상인 친구들만 출력하도록. 70점 입력하면 70점 보다 높은친구들 보여주기 
			//6.분석은 점수들 입력하면 평균점수: 85 최고점수:90점 
...
//				
				...
				for(int i = 0; i < friends.length; i++) {
					if(friends[i] != null && friends[i].name.equals(name)) {
						...
						friends[i].score = (score != -1) ? score : friends[i].score; // 사용자가 안바꾸면 안 바꾼값그대로. 바꾸면 바꾼값이 스코어에 들어감.(초기값을 0으로 할당한 이유는 값이 변경되었는지 확인하려고,0이 아니면 바뀌었다는 의미)
			...
			case 6 : //6.분석: 점수들 입력하면 평균점수:85 최고점수:90점 	
				...
				int cnt = 0, max = 0; // int cnt = 0; int max = 0; 두번 쓰는걸 한번에 쓸수있음
				double avg = 0, sum = 0; // double avg = 0; double sum = 0;
				...
				// System.out.printf("최고점수는 : %d점 ", max); System.out.printf("평균점수는 : %.1f점" ,avg );
				break; // case에 대한 종료

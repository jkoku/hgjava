...
// 학생의 정보를 관리하는 기능 구현.
//기능을 잘 활용하기 위한 용도 StudentApp 필드 생성자  멤버.
...
//생성자.static ㄴㄴ
	...
	//메소드 기능은 메소드로 구현 CRUD
	//등록기능 여기서 구현한 기능은 main이라는 기능에서 호출 할 예정 
	//괄호안의 매개변수 활용 student라고하는 4개의 값을 매개변수 4개 넣거나(String sno,String sname..) 
	//뒤처럼 넣기  ex. (Student std)
...
//등록
	public static boolean add (Student std) {
		for(int i = 0 ; i< students.length; i++) {
			if(students[i] ==  null) {
				students[i] = std;
				return true; // 비어있는 공간이 없으면 true반환이 안될 수 있어서 규칙에 어긋나서 오류. return false도 써줄 것
			}
		}
	return false;
	//System.out.println("입력성공!"); 
	//void 반환값은 잘 입력되었으면 필요에따라 메세지는 메인에서 바꿔주는게 코드 중복되지않고 좋다.
	//처리된 결과가 잘되었으면 t/f값으로 값 반환. 메인에서메시지 설정 sysout말고 
...
//수정. 점수만수정 (학생번호- 점수) 두개를 매개값으로 받도록한다. students안의 값을 다 뒤져서 수정.
		public static boolean modify(String sno, int score) {
			for (int i = 0; i < students.length; i++) {
				if(students[i] != null && students[i].getSno().equals(sno)) {  //null이 아니어야하고 가지고온 학생번호랑 매개변수랑 비교해서 같은게 있으면 
					students[i].setScore(score);													//스코어로 값을 바꾸어주야한다.값을 바꿀때는 셋메소드 만들어둠
					return true;  				
				}
			}
			return false; //못바꿈 학생번호중에 그런학생이 없다는 의미
		}
